---
Id: 1029
Title: "Constructing a Stratified Data Audit Plan from Scratch"
...
<p style={{fontStyle: 'italic', color: '#888', marginTop: '1rem', textAlign: 'center'}}>Synthesize quantile thresholds, stratification, sample sizes, and investigation workflows into a complete audit plan.</p>

<p style={{fontStyle: 'italic', color: '#888', margin: '1rem 0 2rem', textAlign: 'center'}}>We've built the mathematical foundations. Now it's time to assemble them into a complete stratified audit plan—from threshold computation to sample selection to investigation summary.</p>
...
**Key Components:**
```
Threshold Computation → Stratification → Sample Sizing →
Selection → Investigation → Summary → Adjustment
```
...
Extreme_High_New     200     0.15    0.13     Critical
Extreme_High_Seas    150     0.08    0.07     High
High_High_New        500     0.12    0.11     Critical
...
```
n_h = n × (N_h × σ_h) / Σ(N_j × σ_j)
```

**Risk-Weighted Allocation:**
```
n_h = n × (N_h × p_h) / Σ(N_j × p_j)
```
...
## Practical Walkthrough: Design a Three-Strata Plan
### The Problem
...
Using power = 80%, α = 0.05, detecting p_h vs p_0 = 0.5%:
...
Compute Quantiles   Day 13-15: Percentiles, ECDF

Define Strata       Day 23-24: Partitioning, Risk Levels

Compute Sample      Day 14: Power Analysis
Sizes

Select Samples      Day 16: Hypergeometric Sampling

Investigate

Summarize Results   Day 18: F1 Score, Metrics

Adjust Thresholds   Day 22: Set Overlap Analysis
...
**Quantile thresholds** create meaningful strata
**Sample allocation** balances precision and cost
**Power analysis** ensures detectable differences
**Investigation priority** focuses resources
**Summary reporting** enables decisions
**Overlay adjustments** close the loop

**Plan stratified, investigate smart!**
