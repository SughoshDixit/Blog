...
# **Day 24: Risk Segmentation - HR-MR-RR-NR as Priors and Costs** ⚖️📊
...
**The insight:** Risk levels encode our **prior beliefs** and **cost preferences**! 💡

---

## Risk Levels as Prior Beliefs 🎲
...
**Notation:**
- π₁ = P(Fraud) = Prior probability of positive class
- π₀ = P(Not Fraud) = Prior probability of negative class
- π₀ + π₁ = 1
...
**Example:**
```
Segment    π₁ (Fraud Rate)    π₀ (Not Fraud)
─────────────────────────────────────────────
HR         0.30 (30%)         0.70 (70%)
MR         0.10 (10%)         0.90 (90%)
RR         0.02 (2%)          0.98 (98%)
...
## Risk Levels as Cost Weights 💰

### Misclassification Costs

Not all errors are equal! We define:

- **C₀₁ (False Positive Cost):** Cost of flagging a legitimate transaction
  - Customer friction, lost business, review time
- **C₁₀ (False Negative Cost):** Cost of missing a fraud
...
**Example:**
```
Segment    C₁₀ (Miss Fraud)    C₀₁ (False Alarm)    Ratio C₁₀/C₀₁
───────────────────────────────────────────────────────────────────
HR         $10,000             $50                  200:1
MR         $5,000              $100                 50:1
RR         $1,000              $200                 5:1
...
**Interpretation:**
- **HR:** Missing fraud is 200× worse than false alarm → Be aggressive!
- **MR:** Missing fraud is 50× worse → Still favor catching fraud
- **RR:** Missing fraud is 5× worse → More balanced
- **NR:** Costs are equal → Standard threshold
...
- If P(Fraud|x) ≥ 0.5, predict Fraud
...
Predict Fraud if:
```
P(Fraud|x) · C₁₀ > P(Not Fraud|x) · C₀₁
```

Rearranging:
```
P(Fraud|x) / P(Not Fraud|x) > C₀₁ / C₁₀
```
...
## Per-Risk Thresholds: Changing Labeling Geometry 📐
...
**Interpretation:**
- **HR (τ* = 0.005):** Flag anything with >0.5% fraud probability
- **MR (τ* = 0.020):** Flag anything with >2% fraud probability
- **RR (τ* = 0.167):** Flag anything with >16.7% fraud probability
- **NR (τ* = 0.500):** Standard 50% threshold
...
```
Segment    Slope = (C₀₁·π₀)/(C₁₀·π₁)
─────────────────────────────────────
HR         (50·0.70)/(10000·0.30) = 0.012
MR         (100·0.90)/(5000·0.10) = 0.18
RR         (200·0.98)/(1000·0.02) = 9.8
NR         (500·0.999)/(500·0.001) = 999
```
...
```
τ_NR = 0.90  (Only flag very high scores)
```
...
## Real-World Application: Per-Risk Threshold Configuration 🔧
...
risk_config = {
    'HR': {
        'prior': 0.30,
        'cost_fn': 10000,  # C₁₀: cost of missing fraud
...
- Class 0 (Not Fraud): N(μ₀, σ²)
- Class 1 (Fraud): N(μ₁, σ²) where μ₁ > μ₀
- Priors: π₀, π₁
...
**When C₁₀ > C₀₁ (missing fraud is worse):**
- λ < 1, ln(λ) < 0
- Threshold shifts left (lower)
- More events flagged

**When π₁ > π₀ (fraud is common):**
- λ smaller
...
## Best Practices for Risk-Based Thresholding ✅
...
✅ **Risk levels encode priors** — different base rates for different segments
...
✅ **NR handling** — special rules for trusted segments
✅ **Gaussian derivation** — τ* = midpoint + cost/prior adjustment

**Match your thresholds to your costs!** ⚖️🎯

**Tomorrow's Preview:** Day 25 - Threshold Pairing Logic and Equivalence Classes 🔗🎯
