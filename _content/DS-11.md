...
**Enter [Kernel Density Estimation](/key#kernel-density-estimation) (KDE)**: The smooth, elegant solution.
...
**"Let's place a small, smooth hill ([kernel](/key#kernel)) at each data point, then add them all up!"**
...
- **f̂(x)** = estimated [density](/key#density) at point x
- **n** = number of data points
- **K(·)** = kernel function (usually Gaussian/normal curve)
- **xᵢ** = the i-th data point
- **h** = [bandwidth](/key#bandwidth) (controls width of each hill)
...
## The Critical Choice: [Bandwidth](/key#bandwidth) (h)
[Bandwidth](/key#bandwidth) is the **most important** parameter in KDE. It controls how smooth your curve is.
...
## The [Bias-Variance Tradeoff](/key#bias-variance-tradeoff)
...
**Two peaks clearly visible!** This is [bimodal](/key#bimodal) data.
...
You'd conclude this is [unimodal](/key#unimodal) (one group) when it's actually two distinct populations!
...
>  **Note:** This article uses technical terms like [bandwidth](/key#bandwidth), [kernel](/key#kernel), [bias-variance tradeoff](/key#bias-variance-tradeoff), [multimodality](/key#multimodality), and [density estimation](/key#density-estimation). For definitions, check out the [Key Terms & Glossary](/key) page.