...
Topic: Data Science
Abstract: "Learn to view event tagging as rule-based classification. Understand indicator functions, piecewise partitions, and risk-level conditioning—essential tools for mathematically partitioning events into Above The Line (ATL) and Below The Line (BTL) categories."
HeaderImage: /DS-23/label_postprocessing_intro.png
isPublished: true
---

# **Day 23: Label Post-Processing: Partitioning ATL vs BTL Mathematically** 🏷️📊
...
- Which events go **Above The Line (ATL)** → Require human review
- Which events stay **Below The Line (BTL)** → Auto-approved or declined
...
ATL (score ≥ 50): [67, 89, 55, 78] → 4 events (50%)
BTL (score < 50):  [23, 45, 12, 34] → 4 events (50%)
```
...
This means every event is either ATL or BTL—never both, never neither!
...
```
ATL_rule(x, risk) = {
    𝟙{x ≥ 30},  if risk = "Low"     (lower threshold for low risk)
    𝟙{x ≥ 50},  if risk = "Medium"  (standard threshold)
    𝟙{x ≥ 70},  if risk = "High"    (higher threshold for high risk)
}
```
...
```
ATL(x) = 𝟙{R(x) = "Low"} · 𝟙{x ≥ 30}
       + 𝟙{R(x) = "Medium"} · 𝟙{x ≥ 50}
       + 𝟙{R(x) = "High"} · 𝟙{x ≥ 70}
```

**Interpretation:**
- Low-risk events: ATL if score ≥ 30
- Medium-risk events: ATL if score ≥ 50
- High-risk events: ATL if score ≥ 70
...
**Output:**
```python
# Event 1: score=45, risk=Low, threshold=30 → 45 ≥ 30 → ATL
# Event 2: score=55, risk=Medium, threshold=50 → 55 ≥ 50 → ATL
# Event 3: score=65, risk=High, threshold=70 → 65 < 70 → BTL
# Event 4: score=35, risk=Low, threshold=30 → 35 ≥ 30 → ATL
# Event 5: score=75, risk=High, threshold=70 → 75 ≥ 70 → ATL

# Results:
# ATL: [Event 1, Event 2, Event 4, Event 5] → 4 events (80%)
# BTL: [Event 3] → 1 event (20%)
```
...
```
ATL₁(x) = 𝟙{x ≥ t₁}
ATL₂(x) = 𝟙{x ≥ t₁} · 𝟙{x ≥ t₂}  (where t₂ > t₁)
```

Then: `ATL₂(x) ≤ ATL₁(x)` for all x.
...
For all x: `ATL₂(x) ≤ ATL₁(x)` ✓

**Interpretation:** Adding the extra condition `x ≥ t₂` (where t₂ > t₁) can only make it **harder** to be ATL, never easier. This is the **monotonicity property** of conjunctive clauses.
...
```
Before: ATL = 𝟙{score ≥ 50}
After:  ATL = 𝟙{score ≥ 50} · 𝟙{amount ≥ 1000}
```

**Guaranteed:** New ATL ⊆ Old ATL (monotonicity)
...
3. Measure `|ATL_control ⊕ ATL_treatment|` (symmetric difference)
...
✅ **Risk-level conditioning** allows segment-specific thresholds
...
✅ **Conjunctive clauses** can only shrink ATL (monotonicity)

**Think mathematically about your rules!** 🧮🎯

**Tomorrow's Preview:** Day 24 - Risk Segmentation: HR-MR-RR-NR as Priors and Costs ⚖️📊
